...
from ...lib.measurement import filters_to_mongo_selection
from ...lib.util import get_value, group_by
...
def get_regressions(user, project, view, max_ratio):
    observed = view['yAxes']
    if not observed:
        return []
    if not view['watches']:
    ...
    measurements = list(MeasurementRepo(app).get_measurements(
        user,
        project,
        filters_to_mongo_selection(view['filters']), 1000))
...
def check_regressions(user, project):
    view_repo = ViewRepo(app)

    regressions = []
    for view in view_repo.get_views_with_watches(user, project):
        regressions += get_regressions(user, project, view, 1.10)
    return regressions
